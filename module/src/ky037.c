...
// KY-037的参考值，可能需要根据实际模块调整
#define KY037_REF_VOLTAGE 3.3f       // 参考电压（ADC的3.3V）
#define KY037_REF_SOUND_PRESSED_POWER 1.0f // 参考声压（1毫瓦）
#define SOUND_REF_PRESSURE 20E-6f    // 参考声压（20微帕斯卡）
...
float adc_to_decibel(uint16_t adc_value) {
    // 将ADC值转换为电压值
    float voltage = (adc_value / 4096.0f) * KY037_REF_VOLTAGE;

    // 假设模块的灵敏度为-30dB在参考电压下
    float sensitivity = pow(10.0f, (-30.0f) / 20.0f); // 将dB转换为功率比
    float sound_pressure = voltage / KY037_REF_VOLTAGE * sensitivity; // 计算声压

    // 转换为分贝值
    float sound_pressure_level = 20.0f * log10(sound_pressure / SOUND_REF_PRESSURE);

    if (sound_pressure_level < -100.0f) {
        return -100.0f; // 处理非常低的值，避免负无穷
    }
