...
{
    exti_interrupt_disable(FLAME_EXIT_X);
    nvic_irq_disable(FLAME_EXTI_IRQ);
    rcu_periph_clock_disable(RCU_SYSCFG);
    rcu_periph_clock_disable(FLAME_DI_RCU);

    adc_disable(FLAME_ADC);
    rcu_periph_clock_disable(FLAME_AI_RCU);
    rcu_periph_clock_disable(FLAME_ADC_RCU);
}
